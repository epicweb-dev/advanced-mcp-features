...
<EpicVideo url="https://www.epicai.pro/workshops/advanced-mcp-features-flccv/intro~2nnka" />
...
1-64 characters using ASCII letters, digits, `_`, `-`, `.`, and `/`. Use the
top-level `title` for user-facing display and `name` for the stable
...
For a full list of available annotations and their meanings, see the [official MCP documentation on tool annotations](https://modelcontextprotocol.io/specification/2025-11-25/schema#toolannotations).

<callout-info>
	The current spec includes optional `icons` metadata for tools, but the
	high-level TypeScript SDK `registerTool` helper used in this workshop exposes
	`title`, `description`, schemas, annotations, and `_meta`. Keep this exercise
	focused on safety annotations until icon metadata is available through that
	helper.
</callout-info>
...
MCP defaults embedded schemas to JSON Schema 2020-12. The TypeScript SDK lets us
define `outputSchema` with Zod raw shapes in `registerTool`, then exposes the
generated JSON Schema to clients. Tool results put machine-readable data in
`structuredContent`, while `content` remains the user/model-facing response.
...
Validation failures should be surfaced as tool execution errors that the model
can see, generally with `isError: true`, rather than as JSON-RPC protocol
errors. Reserve protocol errors for malformed requests, unknown methods/tools,
and server failures that prevent normal tool execution.

For more details, see the 📜 [official MCP documentation on structured content and output schemas](https://modelcontextprotocol.io/specification/2025-11-25/server/tools#structured-content).
