...
the
[Model Context Protocol (MCP)](https://modelcontextprotocol.io/specification/2025-11-25)—an
...
<callout-info>
	Advanced MCP features often land in clients at different speeds. Verify
	support in your target host before depending on features like elicitation,
	sampling, roots, progress, cancellation, resource subscriptions, or
	`list_changed` notifications in production.
</callout-info>

<callout-info>
	Some server-to-client requests, including `sampling/createMessage`,
	`elicitation/create`, and `roots/list`, are expected to be associated with an
	originating client request such as `tools/call`, `resources/read`, or
	`prompts/get`. Treat these as nested interactions instead of background
	pushes.
</callout-info>

<callout-info>
	HTTP transports are also evolving toward more stateless deployment patterns.
	The exercises here focus on advanced feature behavior, not transport
	architecture, so only adapt transport code if the SDK requires it for your
	runtime.
</callout-info>
