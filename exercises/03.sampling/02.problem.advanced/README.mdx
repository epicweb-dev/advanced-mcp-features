...
<EpicVideo url="https://www.epicai.pro/workshops/advanced-mcp-features-flccv/advanced-sampling-prompt~qyf7c" />
...
<callout-info>
	Sampling with tools is available in the current protocol for clients that
	advertise `clientCapabilities.sampling.tools`. That flow adds `tools` and
	`toolChoice` to the sampling request, handles `ToolUseContent`, runs the
	selected tool server-side, then sends `ToolResultContent` back into a
	follow-up sampling call. We'll keep this step focused on structured sampling
	output and leave sampling-with-tools as an advanced extension.
</callout-info>
