...
<EpicVideo url="https://www.epicai.pro/workshops/advanced-mcp-features-flccv/intro~s3qik" />
...
<callout-info>
	These notifications are not superseded by protocol Tasks. Tasks help with
	durable long-running operations; `list_changed` notifications and resource
	subscriptions still tell clients when definitions or resource contents change.
</callout-info>
...
Metadata updates count too. If a supported metadata field such as a resource
`title`, `description`, or `icons` changes, send the relevant `list_changed`
notification so clients can refresh their cached display data. The current
high-level TypeScript SDK exposes `icons` for resources and resource templates;
tool and prompt icon metadata may require lower-level APIs until helper support
lands.
...
<callout-info>
	Subscriptions are connection/session sensitive. If you deploy with a stateless
	or sessionless HTTP transport, subscription state must live somewhere durable
	or be re-established by the client on each relevant connection.
</callout-info>
...
- 📜 [Server Spec: Resources](https://modelcontextprotocol.io/specification/2025-11-25/server/resources)
- 📜 [Server Spec: Tools](https://modelcontextprotocol.io/specification/2025-11-25/server/tools)
- 📜 [Server Spec: Prompts](https://modelcontextprotocol.io/specification/2025-11-25/server/prompts)
