...
Elicitation should happen while handling an originating client request like this
tool call, not from a detached background job. If the interaction needs to send
the user to a third-party page for OAuth, payments, or API key setup, use URL
mode instead so secrets do not pass through the MCP client.
